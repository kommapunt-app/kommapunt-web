...
interface ValueLibraryBubbleCardProps {
  value: ValueLibraryEntry;
  onSelect: (value: ValueLibraryEntry) => void;
}

export function ValueLibraryBubbleCard({
  value,
  onSelect,
}: ValueLibraryBubbleCardProps) {
  return (
    <button
      type="button"
      onClick={() => onSelect(value)}
      className="group flex aspect-square w-[8.75rem] flex-col items-center justify-center rounded-full border-4 border-komma-black bg-[#F5F5F0] px-3 py-3 text-center shadow-[4px_4px_0_0_#000] transition-all hover:-translate-y-1 hover:bg-white hover:shadow-[5px_5px_0_0_#FF1493] focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-2 focus-visible:outline-komma-pink sm:w-[9.75rem] sm:px-4 sm:py-4"
    >
      <p
